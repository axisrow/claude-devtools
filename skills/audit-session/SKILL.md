...
Runs the token-audit CLI from a claude-devtools checkout (plugin root = repo root). If `pnpm analyze:session` fails with "no such script", cd to this plugin's checkout first and run `pnpm install` once.
